  ...
  return (
    <div className="p-4 pb-24 space-y-4 max-w-lg mx-auto">
      <header className="pt-2">
        <p className="text-xs font-semibold uppercase tracking-wider text-sky-600">Orlando Trip 2026</p>
      ...
      {isBeforeTrip(key) && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 text-center shadow-sm">
          ...
          <p className="text-4xl font-extrabold text-sky-600 mt-1">{countdown.days} días</p>
            ...
            <>
              ...
              <div className="mt-3 flex items-center gap-2">
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-sky-500 rounded-full transition-all" style={{ width: `${todayItems.length ? (doneCount / todayItems.length) * 100 : 0}%` }} />
                </div>
                <span className="text-xs text-slate-500 shrink-0">{doneCount}/{todayItems.length} hecho</span>
              </div>
              <Link to={`/itinerario/${todayPlan.date}`} className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-sky-600 hover:underline">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {QUICK_NAV.map(n => (
            <Link key={n.label} to={n.to} className="tap-scale bg-white rounded-xl border border-slate-200 p-3 flex flex-col items-center gap-1.5 shadow-sm hover:border-sky-300 transition">
              <n.icon size={20} className="text-sky-600" />
